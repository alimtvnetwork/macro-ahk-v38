...
| 2a | ~~**Phase 2a — PascalCase rollout: background runtime consumers**~~ — ✅ **Closed 2026-05-16**: `check-pascalcase-instruction-migration.mjs` CHECK C (every `instruction.json` reader uses PascalCase) passes across all 1195 source files — `auto-injector`, `manifest-seeder`, `default-project-seeder`, `project-matcher`, `script-resolver`, `injection-handler`, `builtin-script-guard`, `namespace-cache`, `url-matcher`, `generate-seed-manifest.mjs` all migrated. CI guard locks the state. | ~~Medium~~ | `mem://standards/pascalcase-json-keys` |
| 2b | ~~**Phase 2b — PascalCase rollout: options UI + popup + scripts + tests**~~ — ✅ **Closed 2026-05-16**: same CHECK C sweep confirms UI/popup/tests also read canonical PascalCase. Phase 2c (drop dual-emit) was predicated on this and is itself ✅; therefore 2b is implicitly complete. | ~~Medium~~ | same |
